...
class TextAnimator {
  ...
  async type(text) {
    ...
    // Split text into words and spaces to preserve word-level wrapping.
    // Each word is wrapped in an inline-block nowrap container so the browser
    // only line-breaks between words, not mid-word.
    ...
    for (const token of tokens) {
      ...
      // For actual words, create a wrapper span that keeps the word together
      let wordWrapper = null;
      if (!isWhitespace) {
        wordWrapper = document.createElement('span');
        wordWrapper.style.display = 'inline-block';
        wordWrapper.style.whiteSpace = 'nowrap';
        if (this._cursor) this.el.insertBefore(wordWrapper, this._cursor);
        else this.el.appendChild(wordWrapper);
      }
      ...
      for (let i = 0; i < token.length; i++) {
        ...
        if (/\s/.test(ch)) { span.classList.add('space'); span.textContent = ch; }
        else span.textContent = ch;

        if (wordWrapper) {
          wordWrapper.appendChild(span);
        } else {
          // Whitespace tokens go directly into the parent
...
function animateCounter(el, target, duration = 1200) {
  // Callers pass the result of getElementById, and the element can be gone
  // before the animation starts (or be torn out mid-flight by an SPA route
  // change) — both threw "Cannot set properties of null" on every run.
  if (!el) return;
