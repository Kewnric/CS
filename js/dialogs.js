...
/* ── The Alert dialog ─────────────────────────────────────────
   A heavier, plainer confirm for the one decision on the practice screen that
   cannot be taken back: finishing an attempt records it and stops the clock.
   The standard confirm puts two same-sized buttons side by side, and the
   difference between them is only the words on them — easy to click through on
   the way to somewhere else. Here the title sits on its own, the reason sits in
   the panel below it, and the two answers are shapes rather than labels: a
   target to go ahead, a cross to back out. */

function showAlertDialog(title, message, onConfirm, opts) {
  const o = opts || {};
  let ov = document.getElementById('alrt-overlay');
  if (!ov) {
  ...
  ov.innerHTML =
    '<div class="modal-content alrt-box" role="alertdialog" aria-modal="true" aria-label="' + escapeHTML(title) + '">' +
      '<div class="alrt-head"><h2>' + escapeHTML(title) + '</h2></div>' +
      '<div class="alrt-body"><p id="alrt-msg">' + escapeHTML(message) + '</p></div>' +
      '<div class="alrt-actions">' +
        '<button class="alrt-btn alrt-ok" id="alrt-ok" title="' + escapeHTML(okLabel) + '" aria-label="' + escapeHTML(okLabel) + '">' +
          '<svg viewBox="0 0 100 100" aria-hidden="true" focusable="false">' +
            '<circle cx="50" cy="50" r="46" fill="none" stroke="#3b7fc0" stroke-width="6"/>' +
            '<circle cx="50" cy="50" r="36" fill="#3b7fc0"/>' +
            '<circle cx="50" cy="50" r="19" fill="#ffffff"/>' +
            '<circle cx="50" cy="50" r="10" fill="#3b7fc0" stroke="#1f4d78" stroke-width="2.5"/>' +
          '</svg>' +
        '</button>' +
        '<button class="alrt-btn alrt-no" id="alrt-no" title="' + escapeHTML(noLabel) + '" aria-label="' + escapeHTML(noLabel) + '">' +
          '<svg viewBox="0 0 100 100" aria-hidden="true" focusable="false">' +
            '<circle cx="50" cy="50" r="46" fill="none" stroke="#c9265d" stroke-width="6"/>' +
            '<circle cx="50" cy="50" r="36" fill="#c9265d"/>' +
            // Drawn twice: the dark pass underneath gives the white cross the
            // thin outline it has against the fill.
            '<path d="M36 36 L64 64 M64 36 L36 64" stroke="#8e1741" stroke-width="15"/>' +
            '<path d="M36 36 L64 64 M64 36 L36 64" stroke="#ffffff" stroke-width="10"/>' +
          '</svg>' +
        '</button>' +
      '</div>' +
    '</div>';
  ...
  const confirm = () => { close(); if (typeof onConfirm === 'function') onConfirm(); };

  function onKey(e) {
    if (ov.classList.contains('hidden')) return;
    if (e.key === 'Escape') { e.stopPropagation(); e.preventDefault(); close(); }
    // Enter only counts on the button that has focus — a blind Enter is exactly
    // the reflex this dialog exists to interrupt.
    else if (e.key === 'Enter' && document.activeElement === document.getElementById('alrt-ok')) {
      e.stopPropagation(); e.preventDefault(); confirm();
    }
  }

  document.getElementById('alrt-ok').onclick = confirm;
  document.getElementById('alrt-no').onclick = close;
  ov.onclick = (e) => { if (e.target === ov) close(); };
  document.addEventListener('keydown', onKey, true);

  if (typeof _trapFocus === 'function') _trapFocus(ov);
  // Focus lands on the way out, not the way through.
  const no = document.getElementById('alrt-no');
  if (no) no.focus();
}
