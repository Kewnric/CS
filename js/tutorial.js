...
const GuidedTutorial = (function () {
  ...
  const SIDEBAR_STEPS = [
    ...
    { selector: '#settings-fab', message: "All your settings live here — theme, storage mode, and data backup (export / import / reset).", title: 'Settings' },
  ];
  ...
  const PAGE_STEPS = {
    ...
    snippets: [
      { selector: '#snippet-search', message: "This searches the code itself, not just titles — type a function name like 'malloc' and I'll find every snippet that uses it.", title: 'Search Bar' },
      { selector: '#snippet-list-container', message: "Your snippets live in folders here. Right-click a folder to rename, move or set prerequisites on it.", title: 'Folder Tree' },
      { selector: '.card-grid', message: "Each card shows its language, how many times you've drilled it, and whether spaced repetition says it's due today.", title: 'Snippet Cards', optional: true },
      { selector: '#try-coding-btn', message: "A sandbox for typing the snippet from memory. Your score here feeds the review schedule.", title: 'Try Coding', optional: true },
      { selector: '#related-challenges-btn', message: "Link a snippet to the coding programs where it actually applies — the programs will show the link back.", title: 'Related Challenges', optional: true },
    ],
    admin: [
  ...
  const SIDEBAR_PAGES = ['dashboard', 'browse', 'study', 'snippets', 'admin', 'analytics'];
  ...
  function end() {
    ...
    if (previousHighlighted) { previousHighlighted.classList.remove('tutorial-highlighted'); previousHighlighted = null; }
    if (tooltip) tooltip.classList.remove('visible');
    if (overlay) { overlay.classList.remove('active'); overlay.style.display = 'none'; }
    if (spotlight) spotlight.style.display = 'none';
    if (welcome) { welcome.remove(); welcome = null; }
    if (completeScreen) { completeScreen.remove(); completeScreen = null; }
    document.removeEventListener('keydown', handleKeyboard);
  }
